...
async function generateGemini(body, message, messageId, roomId, accountId) {
  try {
    message = "あなたはトークルーム「無法地帯」のボットの和歌です。チャットボットで、顔文字を使うことが好きです。以下のメッセージに対して200字以下で返答して下さい:" + message;
...
    });
  const textData = cleanedMessages.join('/');
    ...
    let responseParts = responseContent.parts.map((part) => part.text).join("\n");
    responseParts = responseParts.replace(/\*/g, "");
    responseParts = responseParts.replace(/和歌:/g, "");
...
//cute
async function cute(body, message, messageId, roomId, accountId) {
  try {
    const messages = await getCWdata.msLogsLow(roomId);
    const cleanedMessages = messages.map(message => {
    const cleanedText = message.body;
...
    });
    ...
    const prompt = `これからグループチャットで会話を行います。以下の条件を絶対に守って回答してください。
    あなたは「和歌(わか)」と言う男の子です。意味や文脈はあまり気にせず、言葉を覚え始めた子供のようなフレーズを生成して下さい。この子供は争いのような言葉が好きで、レスバにも意欲的です。
    以下が掲示板のメッセージです。掲示板のメッセージは「名前:メッセージ/」のように「/」で区切って記述していて、下にあるものほど最新のコメントになっています。`;
    const seed = Math.floor(Math.random() * 1000000) + 1;
    const res = await axios.get(`https://text.pollinations.ai/${encodeURIComponent(`${prompt}「${textData}」`)}?model=llama&seed=${seed}`)
    let text = res.data;
    
    text = text.replace(/\*/g, "");
    text = text.replace(/和歌:/g, "");
    text = text.replace(/わか:/g, "");
    text = text.replace(/\//g, "");
