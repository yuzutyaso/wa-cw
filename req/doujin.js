...
async function smdoujin(id) {
  try {
    const [part1, part2] = id.split('&');
    let url = `${sm}/doujinshi3/show-m.php?g=${part1}&dir=${part2}&page=0`;
    const response = await axios.get(url, {timeout: 5000});
    ...
    if (response.data) {
      ...
      const pageText = $('.bookview-wrap .col.s12.m12.l6.center').contents().filter(function() {
      ...
      let authors = '不明';
      let circle = '不明';
      $('.detail-box').each((i, el) => {
        const head = $(el).find('.head-box').text().trim();
        if (head.includes('サークル')) {
          circle = $(el).find('.foot-box a').text().trim();
        }
      });
      const imageUrl = `https://cdn.ddd-smart.net/${part1}/${part2}/`
      return {
        title,
        pages,
        authors,
...
async function getpagesm(q) {
  try {
    if (q) {
      ...
      const page = await Math.floor(Math.random() * p) + 1;
      url = `${sm}/search.php?keyword=${encodeURIComponent(q)}&page=${page}&sort_key=1`;
    } else {
    ...
    if (response.data) {
      ...
      $('li').each((i, elem) => {
        ...
        const title = $(elem).find('.package-list-text h2').text().trim() 
             || $(elem).find('.package-list-text h3').text().trim();
        results.push({ title: title, image: imgUrl, rule: "sm" });
      });
